      ...
      <?php $body = field_collection_item_load($body['value']);?>
        ...
        <div class="post-item">
          <h2><?php print $body->field_boletin_body_subtitulo['und'][0]['value'] ?></h2>
            <?php print check_markup($body->field_boletin_body_texto['und'][0]['value'], $body->field_boletin_body_texto['und'][0]['format']) ?>
        </div>
      <?php else:?>
...
<?php print views_embed_view('agenda', 'eventos_boletin', $node->nid) ?>


<?php if ($node->field_boletin_libros['und'][0]): ?>
  <hr class="hr-dark">
  <h3 class="mb-2"><?php print t('Libros recomendados') ?>:</h3>
  <?php print render($content['field_boletin_libros']) ?>
<?php endif; ?>